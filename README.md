...
| `-i`, `--input` <br><br> or <br><br> `-a`, `--assembly` | Directory containing either extracted sequence regions (`--input`), referring to sequences of the region of interest already isolated from a genome assembly <br><br> **or** <br><Br> complete genome assembly FASTA files (`--assembly`).                                                         | `-i /path/to/region` <br> `-a /path/to/assembly`                                 |
