...
This is the main tool or set of scripts that is used to generate annotation to discover novel and known VDJ segments present it the input data. It takes a list of regions and a library with the help of config file it generates different [annotation results](../README.md#annotation). By default, it uses minimap2, bowtie and bowtie2 to do annotation. This is a quick and simple overview of the core analysis it does.
...
Following the BLAST analysis, the script writes initial Excel file `blast_result.xlsx` and validates the identified segments using the RSS script.
...
With a dynamic command construction for executing mapping processes tailored to specific accuracy requirements and conditions. For example, [`make_bowtie_command`](../scripts/mapping.py#L130) and [`make_bowtie2_command`](../scripts/mapping.py#L170), are able to change their command based on the parsed acc score. Changing the parameters in these function will result in other mapping results. If the

The VDJ sequence themselves is extracted with [`get_sequence`](../scripts/mapping.py#get_sequence). They are retrieved from the FASTA file based on coordinates provided in a BED file. The function, [`get_region_and_segment`](../scripts/mapping.py#L68), categorizes data into biological regions and segments based on predefined [`criteria`](../config/config.yaml#L42), adding structure to the analysis. Important that the headers in the library are separated with a `"_"` and that the region and segment are joined, like this **`TRAV`**.
...
The script then constructs a record dictionary from the library FASTA file through the [`make_record_dict`](../scripts/write_annotation_report.py#L17) function. This dictionary is vital for referencing the library VDJ sequences.
...
Then we divide the entries in `blast_results.xlsx` in to two groups, with the [`main_df`](../scripts/write_annotation_report.py#L80) function. It is split in a dataset with only novel entries, which have at least one mismatch, and a dateset with only known entries containing entries.
...
To validate the entries in the `annotation_report.xlsx` and `annotation_report_100%.xlsx` files, we use the RSS of the found segments, we use `RSS.py`. It process and analyse RSS from the `annotation_report_100%.xlsx`.

This time the config file is really important. It is used to determine how the RSS should be extracted and what the length is of the RSS. This is different per VDJ segment on the different region for TCR and IG. Please read the [`Important configuration settings`](../README.md#important-configuration-settings) for a better understanding on how it works and how to alter it. Using the [`load_config`](../scripts/RSS.py#L22) function these settings are loaded.
...
Tool to nicely visualize VDJ segments on their haplotypes. Giving the ability to show the two haplotypes at once or all single. The tool also has the ability to redetermine to order of the segments using **`--re-evaluate`**, this option is still in beta.
