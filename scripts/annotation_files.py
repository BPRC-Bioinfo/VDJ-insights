...
cwd = os.getcwd()
start, stop = 100, 0  # Adjust the range as needed
reference = os.path.join(cwd, "library", "retained.fasta")
...
def process_files_for_accuracy(accuracy, fasta_files):
    initial_directory = os.path.join(cwd, "segments_mapping", f"{accuracy}%acc")
    primary = os.path.join(initial_directory, "primary_mapping")
    secondary  = os.path.join(initial_directory, "secondary_mapping")
    threads = 10
    ...
    for fasta in fasta_files:
        ...
        fasta = os.path.join(cwd, region_folder, fasta)
        sam_file = os.path.join(primary, f"{prefix}.sam")
        bam_file = os.path.join(primary, f"sorted_{prefix}.bam")
        bed_file = os.path.join(primary, f"{prefix}.bed")
        tags_file = os.path.join(secondary, f"tags_{prefix}.txt")
        tags_fasta_file = os.path.join(secondary, f"mapped_{prefix}.fasta")
        tags_sam_file = os.path.join(secondary, f"mapped_{prefix}.sam")
        tags_bam_file = os.path.join(secondary, f"mapped_{prefix}.bam")
        tags_bed_file = os.path.join(secondary, f"mapped_{prefix}.bed")
        ...
        if not os.path.exists(tags_bed_file):
...
def write_alignments(comparison_results):
    ...
    for prefix in comparison_results:
        ...
        excel_filename = os.path.join(cwd, folder, f"{prefix}_comparative_table.xlsx")
        df.to_excel(excel_filename, index=False)
        logging.info(f"Comparative table for {prefix} saved to {excel_filename}")
    
def main():
    regions = os.path.join(cwd, region_folder)
    fasta_files = [f for f in os.listdir(regions) if f.endswith('fasta')]
